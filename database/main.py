...
def GeraDatabase():
    ...
    while contagem_clima < len(clientes_final):
        cur.executemany('INSERT INTO CLIMA (CLIMA_DATA, CLIMA_CHUVA_MM) VALUES (%s, %s)', clima_final[contagem_clima: passos_clima])
        contagem_clima += 100
        passos_clima += 100
        cnx.commit()
    ...
    while contagem_funcionarios < len(data_mult_funcionarios):
        cur.executemany('INSERT INTO FUNCIONARIOS (FUNCIONARIO_NOME, FUNCIONARIO_DATA, FUNCIONARIO_ATRACAO_ID) VALUES (%s, %s, %s)', funcionarios_final[contagem_funcionarios: passos_funcionarios])
